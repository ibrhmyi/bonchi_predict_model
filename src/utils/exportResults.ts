...
export function exportAsJSON(ranking: RankedConcept[], meta: ExportMeta): void {
  const data = {
    ...
    results: ranking.map((r) => ({
      ...
      costIndex: r.costEfficiency?.costIndex ?? null,
      supplyReliability: r.costEfficiency?.supplyReliability ?? null,
      insights: r.insights,
...
export function exportAsCSV(ranking: RankedConcept[], meta: ExportMeta): void {
  ...
  const rows = ranking.map((r, i) => [
    ...
    r.costEfficiency?.costIndex ?? "",
    r.costEfficiency?.supplyReliability ?? "",
    `"${meta.country}"`,
    `"${meta.base}"`,
    `"${meta.strategy}"`,
    meta.pricePoint,
